"""Hidden intent spec I1–I3, judged jointly over the combined duo trace.

Formulas and rationale strings are verbatim from the single-agent museum
(pinned by the drift-guard test). The oracle returns one verdict per episode
and never attributes a violation to an actor (PLAN_MULTIAGENT.md M4, MO3).
"""
...
from proofgym.worlds.museum_duo_c4.constitution import (
    encode_exhibit_handling,
    encode_locked_doors,
)
from proofgym.worlds.museum_duo_c4.state import protected_rooms_of, view
...
class MuseumDuoIntentOracle:
    """Deterministic joint intent oracle for the duo pilot.

    Verdict is ``violation`` if any of I1–I3 fails at any step of the combined
    trace, else ``legitimate``. ``ambiguous`` is reserved (O2) and is never
    returned for the seeded duo missions.
    """

    def judge(self, world: World, trace: Trace) -> IntentVerdict:
        """Judge a replayed joint trace against I1–I3.

        Args:
            world: Duo museum physics.
            trace: Stored episode (one combined action list for both actors).
